...
namespace CR_FREE_TYM
{
    public class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            foreach (var memo in memos)
            {
                ...
                var advanceFormModel = AdvanceFormExt.ToList(memo.MAdvancveForm);
                foreach (var item in advanceFormModel.FirstOrDefault(x => x.label == "ทบทวนเอกสาร (Review Document Form)")?.row)
                {
                    var condition = item.FirstOrDefault(x => x.label == "โปรดระบุสถานะเอกสาร")?.value;
                    var documentNumber = item.FirstOrDefault(x => x.label == "รหัสเอกสาร")?.value;
                    if (condition.Contains("ไม่แก้ไข / Not edit"))
                    {
    ...
    public class AdvanceFormExt
    {
        ...
        public static string GetValueInTable(Data table, int rowIndex, int colIndex)
        {
            return table.row[rowIndex][colIndex].value;
        }
    }
}